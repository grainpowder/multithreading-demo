...
def start(bound_type: BoundType = typer.Argument(..., case_sensitive=False, help="type of bounded task")):
    ...
    if bound_type == BoundType.CPU:
        ...
        logger.info("Execute CPU-bounded task synchronously")
        synchronous_execute(dimensions)

        logger.info("Execute CPU-bounded task using multithreading")
        multithread_execute(dimensions)

        logger.info("Execute CPU-bounded task using multiprocessing")
        ...
        logger.info("Execute IO-bounded task synchronously")
        SynchronousIOBoundTask.execute(warmup=False)

        logger.info("Execute IO-bounded task using multithreading")
        MultithreadIOBoundTask.execute()

        logger.info("Execute IO-bounded task asynchronously")
